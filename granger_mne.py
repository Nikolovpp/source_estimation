...
from granger import reduce_roi_first_pc, band_average, DEFAULT_BANDS
...
def timefreq_gc_mne(data, fs, cwt_freqs, n_cycles, gc_n_lags, seeds, targets,
    ...
    con = spectral_connectivity_epochs(
        ...
        rank=([rank_val] * n_edges, [rank_val] * n_edges), verbose=False)
    return np.asarray(con.get_data())            # (n_edges, n_freqs, n_times)


def compute_subject_gc_mne(roi_data, times, sfreq, *, gc_n_lags=25, win_ms=250.0,
    ...
    ``(n_epochs, n_vertices, n_times)`` (reduced to one virtual channel via a
    fixed first-PC filter) or already-reduced ``(n_epochs, n_times)``.
    ...
    (the real time axis in ms), ``freqs``, ``fs``, ``fxy``/``fyx`` (band ->
    ``(n_pairs, n_times)``; ``fxy`` = i->j, ``fyx`` = j->i) and, if ``trgc``,
    ``dtrgc`` (band -> ``(n_pairs, n_times)``, Diff-TRGC for i->j).
    """
    ...
    # Reduce each ROI to one virtual channel (no-op if already reduced).
    vcs = []
    for r in roi_names:
        arr = np.asarray(roi_data[r], float)
        vcs.append(reduce_roi_first_pc(arr) if arr.ndim == 3 else arr)
    ...
    # All directed edges in one MNE call.
    edges = []
    for (i, j) in pairs:
        edges += [(i, j), (j, i)]
    seeds = [[e[0]] for e in edges]
    targets = [[e[1]] for e in edges]
    edge_idx = {e: k for k, e in enumerate(edges)}

    data = np.transpose(V, (1, 0, 2))            # (n_ep, n_roi, n_t)
    gc = timefreq_gc_mne(data, fs, cwt_freqs, n_cyc, gc_n_lags,
                         seeds, targets, 'gc', rank_val)
