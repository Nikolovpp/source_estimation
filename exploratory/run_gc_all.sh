...
python exploratory/run_granger_sensor.py --task overtProd  --stim-class all \
    --order $ORDER --win-ms $WIN_MS --target-fs $TARGET_FS --trgc --n-jobs $SENSOR_NJOBS

python exploratory/run_granger_sensor.py --task perception --stim-class all \
    --order $ORDER --win-ms $WIN_MS --target-fs $TARGET_FS --trgc --n-jobs $SENSOR_NJOBS

# Optional: contrast-split (prodDiff / percDiff) instead of all-trials-combined
# python exploratory/run_granger_sensor.py --task overtProd  --stim-class prodDiff --order $ORDER --win-ms $WIN_MS --target-fs $TARGET_FS --trgc --n-jobs $SENSOR_NJOBS
# python exploratory/run_granger_sensor.py --task perception --stim-class percDiff --order $ORDER --win-ms $WIN_MS --target-fs $TARGET_FS --trgc --n-jobs $SENSOR_NJOBS
